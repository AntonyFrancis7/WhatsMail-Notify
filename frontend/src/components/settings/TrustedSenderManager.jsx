...
export default function TrustedSenderManager({ senders, onAdd, onDelete, loading }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEmail) {
      ...
      if (!validateDomain(value)) {
        setError("Please enter a valid domain name (e.g. github.com).");
        return;
      }
    }
    ...
    if (isDuplicate) {
      setError("This sender or domain is already in the list.");
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to add trusted sender.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <SettingsCard
      ...
      subtitle="Emails or domains whitelisted to always bypass categories and trigger notifications."
    >
      <div className="space-y-6">
        <form onSubmit={handleSubmit} className="space-y-2">
          <div className="flex gap-2">
            <input
              type="text"
              placeholder="e.g. boss@company.com or github.com"
              value={newSender}
              onChange={(e) => {
        ...
        <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
          {senders.length === 0 ? (
            <EmptyState message="No trusted senders whitelisted yet." />
          ) : (
            senders.map((s) => (
              <div
                key={s.id}
              ...
              >
                <div className="flex items-center gap-2 truncate">
                  <div className="p-1.5 rounded bg-emerald-500/10 text-emerald-400 flex-shrink-0">
                    {s.email ? (
